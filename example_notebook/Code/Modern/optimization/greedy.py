...
def _greedy_step(population, J):
    """
    Performs one standard greedy step vectorized over the population:
    Finds the spin with the largest energy drop (most negative Delta E) and flips it.
    ...
    # Pick spin with most negative Delta E (largest energy decrease)
    best_spins = torch.argmin(masked_delta_E, dim=1)
...
def greedy_search(L, J, pop_size, num_sweeps, Observables, 
    ...
    mode : str
        ...
        - "greedy": Chooses the downhill move with the largest energy decrease.
    record_interval : int
    ...
    """
    if mode not in ["random", "reluctant", "greedy"]:
        raise ValueError("mode must be 'random', 'reluctant', or 'greedy'")
    ...
    if mode == "random":
        ...
        step_fn = _reluctant_step
    elif mode == "greedy":
        step_fn = _greedy_step
