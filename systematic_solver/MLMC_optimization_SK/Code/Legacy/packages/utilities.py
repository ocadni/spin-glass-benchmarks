...
def TTSp(energies,E_ground_state):
    sucess=np.abs(energies-E_ground_state)<1e-5
    sucess.int()
...
def update(frame):
    ax.clear()
    data = T[frame].numpy()
    ax.hist(data, bins=60, range=(xmin, xmax))
    ax.set_xlim(xmin, xmax)
    #ax.set_yscale("log")
    #ax.set_ylim(1, 300)  # optional: fix y-axis too
    ax.set_title(f"Frame {frame}, T={temperatures[frame-1]:.5f},gamma={Gammas[frame -1] :.5f}")

anim = FuncAnimation(fig, update, frames=len(T), interval=300)

plt.show()
anim.save("histogram_SK_N50.gif", writer="pillow", fps=5)
