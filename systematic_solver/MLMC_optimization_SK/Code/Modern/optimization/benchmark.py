...
from simulated_annealing_SK import *
from quantum_annealing_SK import *
from PT_SK import *
from population_annealing_SK import *
from greedy_SK import *
from global_annealing_SK import *
from monte_carlo import *
path = '../../../../MLMC_optimization_SK/Data/Alpha/Couplings/Configurations/'
...
if __name__ == "__main__":
    ...
    for file in txt_files:
        re.compile(r"sk_couplings_N(\d+)_J0_seed(\d+)\.t$")
        N    = int(re.search(r"N(\d+)_", file).group(1))
        seed = int(re.search(r"seed(\d+)\.", file).group(1))
        J = read_couplings(path+file, N).to(device)
        print(file)
        ...
        if algorithm=='Greedy':
            ...
            #GA
            temperatures, observ, elapsed_time = global_annealing( J, pop_size, 
                                                                  MLMCsteps, MCsteps,N,  Tstart,
            ...
            #PA
            temperatures, observ, elapsed_time = population_annealing(N, J, pop_size, MCsteps, 
        ...
        RESULTS_DIR = Path(__file__).resolve().parent / "../../../../MLMC_optimization_SK/Data/Alpha/Couplings/Results"
        RESULTS_DIR.mkdir(parents=True, exist_ok=True)

        with open(RESULTS_DIR / f"results_SK_algo_{algorithm}_N{N}_seed{seed}.txt", "w") as f:
            f.write(f"N {N}\n")
            f.write(f"seed {seed}\n")
        ...
        print("SAVED: ")
        print(f'results_SK_algo_{algorithm}_N{N}_seed{seed}.txt')
