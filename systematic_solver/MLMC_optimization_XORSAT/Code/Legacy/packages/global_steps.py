...
def generate_config(model, N_spins, N_config, J):
    ...
    with torch.no_grad():
        ...
        config = torch.zeros((N_config, N_spins))
...
def generate(model, current_config, new_beta, N, J,h, N_data = 100000, N_configs = 10):
    """
    ...
    # Initialize acceptance rate counter
        ...
        for t in tqdm(range(N_configs)):
            ...
            energy_old = -torch.einsum("ki,ik->k", old_configs, torch.einsum("ij,kj->ik", J, old_configs)) / 2-torch.einsum("ki,i->k", old_configs, h)
            arg_old = -new_beta * energy_old + torch.sum(bce(model(old_configs), (old_configs + 1) / 2), axis=1)
            
            new_energies = -torch.einsum("li,il->l", new_configs, torch.einsum("ij,lj->il", J, new_configs)) / 2-torch.einsum("ki,i->k", new_configs, h)
            arg_new = -new_beta * new_energies + torch.sum(bce(model(new_configs), (new_configs + 1) / 2), axis=1)
            
            # Acceptance probability calculation
            acc = (torch.log(torch.rand(size=(N_data,))) < (arg_new - arg_old)).int()
