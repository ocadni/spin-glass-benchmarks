...
def MLMC_fast(model, data, beta, N, J,h,  num_steps = 10, return_correlations = False):
    ...
    with torch.no_grad():
        ...
        for t in range(num_steps):
            ...
            current_energy = compute_energy(current_config, J,h)
            current_probability = torch.sum(bce(model(current_config), (current_config+1)/2), axis = 1)

            new_energy = compute_energy(new_config, J,h)
            ...
            acceptances = (torch.log(torch.rand(size=(len(data),))) < (arg_new-arg_current)).int()
...
def global_annealing(L, J,h, pop_size, num_steps_MC, swap_step, N, Tstart, Tend, Observables,
    ...
    for currT in temperatures[1:-1]:
        ...
        for i in range(num_steps_MC):
            population, _ = MLMC_fast(model, population, 1/currT, N, J,h, num_steps=1)
            for j in range(swap_step):
                population = monte_carlo_update_fast(population, J, h,1/currT, INDICES)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--K', type=int, default=99, help='Lattice size')
    ...
    parser.add_argument('--num_temps', type=int, default=50, help='Number of annealing temperatures')
    ...
    temperatures, observ, elapsed_time = global_annealing(K, J,h, pop_size, MLMCsteps, MCsteps,N,  Tstart,
