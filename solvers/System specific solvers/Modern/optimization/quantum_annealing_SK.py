...
def compute_energy_ST(s, J, J_trotter):
    """
    ...
    E_classical = -0.5 * (s * Js).sum(dim=-1)           # [pop, P]
...
def compute_delta_energy_ST(s, proposed, J, J_trotter, indices):
    """
    ...
    field = torch.einsum("j, bpj -> bp", J[indices], s)          # [pop, P, |idx|]
    dE_cl = -2.0 * proposed[:, :, indices] * field                  # [pop, P, |idx|]
...
def mc_update_ST(population, J, beta, J_trotter):
    """
    ...
    for indices in range(N):
        ...
        dE = compute_delta_energy_ST(pop, proposed, J, J_trotter, indices)  # [pop, P, |idx|]

        # Metropolis: accept each spin independently across pop, P, |idx|
        log_rand = torch.log(torch.rand_like(dE))
...
def quantum_annealing_ST(
    ...
    for _ in range(high_temp_therm):
        population = mc_update_ST(population, J, beta0, J_T0)

    # Record initial state (collapse slices by taking mean config or best-energy slice)
    best_slice = _best_slice(population, J)
    ...
    for t, (T, Gamma) in enumerate(zip(temperatures, Gammas)):
        ...
        for _ in range(num_steps_MC):
            population = mc_update_ST(population, J, beta, J_T)
    ...
    best_configs = _best_slice(population, J)   # [pop, N]
...
def _best_slice(population, J):
    ...
    E   = -0.5 * (population * Js).sum(dim=-1)              # [pop, P]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--MCsteps",      type=int,   default=100)
    parser.add_argument("--num_temps",    type=int,   default=50)
    parser.add_argument("--schedule",     type=str,   default="linearT")
    parser.add_argument("--pop_size",     type=int,   default=20000//20)
    ...
    temperatures, observ, elapsed, best_configs = quantum_annealing_ST(
        N=N, J=J,
