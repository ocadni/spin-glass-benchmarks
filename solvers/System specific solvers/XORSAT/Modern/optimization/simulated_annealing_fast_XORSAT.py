...
def simulated_annealing(
    K, J,h, pop_size, num_steps_MC, N, Tstart, Tend, Observables,
    ...
    for _ in range(high_temp_thermalization_steps):
        population = monte_carlo_update_fast(
            population, J,h, 1.0 / temperatures[0],INDICES)
        ...
        if step_idx % thinning == 0:
            ...
            if early_stop_tol is not None:
                ...
                if best_min - current_min < early_stop_tol:
                    plateau += 1
                    if plateau >= 1000:          # 3 consecutive non-improvements → stop
                        break
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pop_size",  type=int,   default=100000,        help="Population size (baseline was 10000)")
    ...
    parser.add_argument("--num_temps", type=int,   default=100,          help="Number of annealing temperatures")
    parser.add_argument("--schedule",  type=str,   default="Cv_beta",   help="Temperature schedule")
